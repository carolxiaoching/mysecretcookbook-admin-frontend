<template>
  <VForm class="col-12 col-xxl-8" ref="form" v-slot="{ errors }" @submit="updateStep(2)">
    <div
      class="d-flex flex-column mb-16 p-16 bg-white rounded shadow-sm"
    ...
    >
      ...
      <div class="flex-fill mx-16" style="min-height: 20rem">
        <UploadComponent :temp-image-url="tempRecipeData.coverImgUrl" @update-image="uploadImage" />
      </div>
    </div>
    <div class="p-16 bg-white rounded shadow-sm mb-16">
      <h2 class="title mb-11">
        <span class="title-icon bg-secondary-subtle">
      ...
      <div class="row g-8">
        ...
        <div class="col-6" v-if="recipe.user">
        ...
        <div :class="[recipe.user ? 'col-6' : 'col-12']">
    ...
    <div class="d-flex align-items-center">
      <router-link to="/dashboard/recipes" class="btn btn-outline-primary me-16">
        <i class="bi bi-arrow-90deg-left me-8"></i>
        返回食譜列表
      </router-link>
      <button type="submit" class="btn btn-primary flex-fill">下一步</button>
    </div>
  </VForm>
</template>
