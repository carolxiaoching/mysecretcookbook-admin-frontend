<template>
    ...
    <table class="table mb-14 align-middle">
      <thead>
        <tr class="table-active">
          ...
          <th class="text-center" width="120">圖片類型</th>
      ...
      <tbody>
        <tr v-for="item in images" :key="item._id">
          <td class="d-none d-sm-table-cell text-center">#{{ item._id.substring(-1, 5) }}</td>
  ...
  <DelModal ref="delModalRef" :del-id="delData.id" @del-item="delImage">
    ...
    <template #modal-content>
      <p class="mb-8">
        確定要刪除 <strong> 圖片：#{{ delData.id.substring(-1, 5) }} </strong>
      </p>
      <img :src="delData.imageUrl" class="img-thumbnail" style="max-height: 12rem" />
    </template>
  </DelModal>
  <ImageModal ref="imageModalRef">
    <template #modal-header> 圖片 #{{ tempImage.id.substring(-1, 5) }} </template>
