  ...
  <div class="container-fluid">
    <div class="row gx-16 justify-content-center">
      ...
      <div class="col-12 col-xxl-6 mb-16">
        <div class="d-flex flex-column h-100 p-16 bg-white rounded shadow-sm">
          ...
          <div class="flex-fill m-16" style="min-height: 20rem">
            <UploadComponent :temp-image-url="member.avatarImgUrl" @update-image="uploadImage" />
          </div>
        </div>
      </div>
      <div class="col-12 col-xxl-6 mb-16">
        <div class="p-16 bg-white shadow-sm rounded">
          <h2 class="title mb-11">
            <span class="title-icon bg-secondary-subtle">
              <i class="bi bi-image-fill"></i>
            </span>
...
function updateMember(item) {
  openLoading();
  const id = member.value._id;

  apiUpdateMember(item, id)
    .then((res) => {
      const { status, data } = res.data;
      if (status === 'success') {
        pushMessage({
...
function uploadImage(imageUrl) {
  updateMember({ avatarImgUrl: imageUrl });
}
