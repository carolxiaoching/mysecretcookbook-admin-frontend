<template>
    ...
    <table class="table align-middle">
      <thead>
        <tr class="table-active">
          ...
          <th class="d-none d-xxl-table-cell text-center" width="120">收藏人數</th>
      ...
      <tbody>
        <tr v-for="item in recipes" :key="item._id">
          <td class="d-none d-md-table-cell text-center">#{{ item._id.substring(-1, 5) }}</td>
  ...
  <DelModal ref="delModalRef" :del-id="delData.id" @del-item="delRecipe">
    <template #modal-header> 刪除食譜 #{{ delData.id.substring(-1, 5) }} </template>
...
import { apiGetRecipes, apiGetCategories, apiDelRecipe } from '@/scripts/api';
import { loadingStore, messageStore } from '@/stores/index';
import Pagination from '@/components/PaginationComponent.vue';
import DelModal from '@/components/DelModal.vue';
import { getPageDataAndPagination, getCategoryName } from '@/scripts/methods';
